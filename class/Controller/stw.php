...
class stw implements Listable {
    ...
    public static function findMailingLog($stwID, $fetchMode=\PDO::FETCH_OBJ) {
		$sql = Sql::select("stwMailingLog")->where(['stwID', '=', $stwID]);
		$stm = $sql->prepare();
		$stm->execute();
		return $stm;
	}     
	...
	public static function getMailingLogDetail($mailingLogID, $fetchMode=\PDO::FETCH_OBJ) {
		$sql = Sql::select("stwMailingLog")->where(['id', '=', $mailingLogID]);
		$stm = $sql->prepare();
		$stm->execute();
		$obj = $stm->fetch($fetchMode);
		if ($obj === false) return null;
    ...
    public function list($request) {
		...
		$obj = null;
		return new FormPage('stw/list', $obj);
	}
    ...
    public function stwForm($request) {
		...
		$formName = "form-addStw";

		if(!is_null($obj)) {
			$formName = "form-editStw";
		}				
                    ...
                    $content .= "<div class='card'>";
                        ...
                        $content .= "<div class='card-header'>";
                            $content .= "<div class='d-flex align-items-center'>";
                                $content .= "<h4 class='card-title' id='notice'>".L('info.stwAddHelperMessage')."</h4>";
                            $content .= "</div>";
                        $content .= "</div>";

                        $content .= "<div class='card-body'>";

                            $content .= "<ul class='nav nav-pills nav-secondary' id='pills-tab-stwAdd' role='tablist'>";

                                $content .= "<li class='nav-item submenu stwAddMenu' role='presentation'>";
                                    $content .= "<a class='stwAddMenuA nav-link active' id='pills-stwapplication-tab' data-bs-toggle='pill' href='#pills-stwapplication' role='tab' aria-controls='pills-stwapplication' aria-selected='false' tabindex='-1'>".L('stw.applicationDetail')."</a>";
                                $content .= "</li>";

                                $content .= "<li class='nav-item submenu stwAddMenu' role='presentation'>";
                                    $content .= "<a class='stwAddMenuA nav-link ' id='pills-mailing-tab' data-bs-toggle='pill' href='#pills-mailing' role='tab' aria-controls='pills-mailing' aria-selected='false' tabindex='-1'>".L('stw.mailingDetail')."</a>";
                                $content .= "</li>";                                

                            $content .= "</ul>";
                            
                            $content .= "<div class='tab-content mt-2 mb-3' id='pills-tabContent-stwAdd'>";

                               // application tab
                                $content .= "<div class='tab-pane stwAddMenuDiv fade show active' id='pills-stwapplication' role='tabpanel' aria-labelledby='pills-stwapplication-tab'>";
                                    ...
                                    $content .= "<div class='row'>";
                                        ...
                                        $content .= formLayout::rowSelectNew(L('stw.client'), 'clientID', 'clientID', $option,  6, ['clientSelect'], ['required'], is_null($obj)?'':$obj['clientID']);  
                                    $content .= "</div>";
                        
                                    $content .= "<div class='row' id='stwSelectedClentDetail'>";
                                    ...
                                    if($tpbID==0){  
                                        $content .= formLayout::rowInputNew(L('stw.addressDDLot'),'addressDDLot', 'addressDDLot', 'text',  12, [], [], is_null($obj)?'':$obj['addressDDLot']);         
                                    } else {
                                        $content .= formLayout::rowInputNew(L('stw.addressDDLot'),'addressDDLot', 'addressDDLot', 'text',  12, [], [], is_null($obj)?$tpbObj->addressDDLot:$obj['addressDDLot']);         
                                    }
                                    ...
                                    $content .= "<div class='row'>";
                                        $content .= formLayout::rowInputNew(L('stw.submissionDoc'),'STWDoc', 'STWDoc', 'file',  6, [], ['accept="image/*, application/pdf"']);
                                        if(isset($obj['STWDocID']) && $obj['STWDocID']>0) {
                                            $content .= formLayout::rowDisplayClearGroupLineNew('<div class="d-flex gap-2 btnGrp"><button type="button" class="btn btn-black downloadDoc btn-xs mt-2" data-id="'.$obj['STWDocID'].'"><i class="fas fa-download"></i></button><button type="button" class="btn btn-danger removeDoc btn-xs mt-2" data-id="'.$obj['STWDocID'].'" data-stw="'.$obj['id'].'" data-doc="STWDocID"><i class="fas fa-trash"></i></button></div>',6);
                                        } else {
                                            $content .= formLayout::rowDisplayClearGroupLineNew('<div class="d-flex gap-2 btnGrp" style="display: none !important"><button type="button" class="btn btn-black downloadDoc btn-xs mt-2" data-id="0"><i class="fas fa-download"></i></button><button type="button" class="btn btn-danger removeDoc btn-xs mt-2" data-id="0" data-stw="'.$obj['id'].'" data-doc="STWDocID"><i class="fas fa-trash"></i></button></div>',6);
                                        }     
                                ...
                                // mailing tab
                                $content .= "<div class='tab-pane stwAddMenuDiv fade' id='pills-mailing' role='tabpanel' aria-labelledby='pills-mailing-tab'>";
                                    $content .= "<div class='row'>";
                                        if($formName == "form-addStw") {
		...
		return new Data(['success'=>true, 'message'=>$content, 'stwID'=>is_null($obj)?'1':$obj['id']]);
    ...
    public function add($request) {	
        ...
        if (!isset($request->post->refNo) || empty($request->post->refNo)) 
			return new Data(['success'=>false, 'message'=>L('error.stwEmptyRefNo'), 'field'=>'refNo', 'tab'=>'pills-stwapplication']);
        ...
        */
        ...
        if (!isset($request->post->submissionDate) || empty($request->post->submissionDate)) 
			return new Data(['success'=>false, 'message'=>L('error.stwEmptySubmissionDate'), 'field'=>'submissionDate', 'tab'=>'pills-stwapplication']);           
        */
        ...
        // insert database
		$sql = Sql::insert('stw')->setFieldValue([
            'refNo' => "?", 
            'tpbID' => "?", 
            'clientID' => "?", 
            'addressDDLot' => "?", 
    ...
    public static function removeDoc($request) {	
        ...
        if (!isset($request->post->stwID) || empty($request->post->stwID))
		    return new Data(['success'=>false, 'message'=>L('error.stwEmptyID')]);	
        
        $stwObj = self::find($request->post->stwID);

        if (is_null($stwObj))
            return new Data(['success'=>false, 'message'=>L('error.stwNotFound')]);	
	
        if(documentHelper::delete($docObj->id)) {

            $sql = Sql::update('stw')->setFieldValue([$request->post->docType => '0'])->where(['id', '=', $request->post->stwID]);
            if($sql->prepare()->execute()){
    ...
    public static function genTableHeader() {
        ...
        $htmlContent .= "<thead>";
            $htmlContent .= "<tr>";
                ...
                $htmlContent .= "<th>".L('stw.refNo')."</th>";
				$htmlContent .= "<th>".L('tpb.number')."</th>";
				$htmlContent .= "<th>".L('stw.client')."</th>";
				$htmlContent .= "<th>".L('stw.addressDDLot')."</th>";
                $htmlContent .= "<th>".L('stw.submissionDate')."</th>";
	...
	public static function genTableBodyRow($listObj) {
        ...
        $htmlContent .= "<tr>";
			...
			$htmlContent .= "<td>".$listObj['stwRefNo']."</td>";
                            ...
                            $content .= "<div class='tab-content mt-2 mb-3' id='pills-tabContent-stwAdd'>";
                               ...
                               // application tab
                                $content .= "<div class='tab-pane stwAddMenuDiv fade show active' id='pills-stwapplication' role='tabpanel' aria-labelledby='pills-stwapplication-tab'>";
                                    ...
                                    $content .= "<div class='row' id='stwSelectedClentDetail'></div>";
    ...
    public function mailingLogdetail($request) {
		...
		$content = "<div class='row'>";     
			...
			$content .= formLayout::rowDisplayLineNew(L('stw.mailingDate'), $obj['mailingDate'], 6);
			$content .= formLayout::rowDisplayLineNew(L('stw.mailingFrom'), $obj['mailingFrom'], 6);
            $content .= formLayout::rowTextAreaNew(L('stw.mailingContent'), 'mailingContent', 'mailingContent',  12, [], ['readonly'], $obj['mailingContent']);        
    ...
    public static function genMailingLogTableHeader() {
        ...
        $htmlContent .= "<thead>";
            $htmlContent .= "<tr>";
                $htmlContent .= "<th>".L('ID')."</th>";
                $htmlContent .= "<th>".L('stw.mailingDate')."</th>";
				$htmlContent .= "<th>".L('stw.mailingFrom')."</th>";
                $htmlContent .= "<th>".L('stw.mailingContent')."</th>";                 
                $htmlContent .= "<th>".L('Actions')."</th>";
            $htmlContent .= "</tr>";
        $htmlContent .= "</thead>";
	...
	public static function genMailingLogTableContentData($stwID) {
		$sql = Sql::select("stwMailingLog")->where(['stwID', '=', $stwID]);   
        $stm = $sql->prepare();
        $stm->execute();
        return $stm;
            ...
            $content .= "<div class='row'>";
                $content .= "<div class='col-md-12'>";
                    $content .= "<div class='card'>";
                        ...
                        $content .= "<div class='card-header'>";
                            $content .= "<div class='d-flex align-items-center'>";
                                $content .= "<h4 class='card-title' id='notice'>".L('info.stwMailingLogAddHelperMessage')."</h4>";
                            $content .= "</div>";
                        $content .= "</div>";

                        $content .= "<div class='card-body'>";   
                            $content .= "<div class='row'>";                        
                                $content .= formLayout::rowInputNew(L('stw.id'),'stwID', 'stwID', 'hidden',  6, [], [], $request->get->id); 
                            $content .= "</div>";                            
                            $content .= "<div class='row'>";
                                $content .= formLayout::rowInputNew(L('stw.mailingDate'),'mailingDate', 'mailingDate', 'text',  6, ['customDateTime'], ['required']);
                                $content .= formLayout::rowInputNew(L('stw.mailingFrom'),'mailingFrom', 'mailingFrom', 'text',  6, [], ['required']);                
                            $content .= "</div>";

                            $content .= "<div class='row'>";  
                                $content .= formLayout::rowTextAreaNew(L('stw.mailingContent'), 'mailingContent', 'mailingContent',  12, [], []);                                         
                            $content .= "</div>";
    ...
    public function mailingLogAdd($request) {
        ...
        if (!isset($request->post->mailingDate) || empty($request->post->mailingDate)) 
			return new Data(['success'=>false, 'message'=>L('error.stwEmptyClient'), 'field'=>'mailingDate']);

        if (!isset($request->post->mailingFrom) || empty($request->post->mailingFrom)) 
			return new Data(['success'=>false, 'message'=>L('error.stwEmptyClient'), 'field'=>'mailingFrom']);            
            ...
            $content .= "<div class='row'>";
                $content .= "<div class='col-md-12'>";
                    $content .= "<div class='card'>";
                        ...
                        $content .= "<div class='card-body'>";   
                            $content .= "<div class='row'>";
                                $content .= formLayout::rowInputNew(L('stw.mailingDate'),'mailingDate', 'mailingDate', 'text',  6, ['customDateTime'], ['required'], is_null($obj)?'':$obj['mailingDate']);
                                $content .= formLayout::rowInputNew(L('stw.mailingFrom'),'mailingFrom', 'mailingFrom', 'text',  6, [], ['required'], is_null($obj)?'':$obj['mailingFrom']);                
                            $content .= "</div>";

                            $content .= "<div class='row'>";  
                                $content .= formLayout::rowTextAreaNew(L('stw.mailingContent'), 'mailingContent', 'mailingContent',  12, [], [], is_null($obj)?'':$obj['mailingContent']);                                         
                            $content .= "</div>";
