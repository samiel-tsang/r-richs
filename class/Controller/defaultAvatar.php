...
class defaultAvatar {
	...
	public static function find($id, $fetchMode=\PDO::FETCH_OBJ) {
		$sql = Sql::select("defaultAvatar")->where(['id', '=', "?"]);
		$stm = $sql->prepare();
		$stm->execute([$id]);
	...
	public static function find_by_url($url, $fetchMode=\PDO::FETCH_OBJ) {
		$sql = Sql::select("defaultAvatar")->where(['url', '=', "?"]);
		$stm = $sql->prepare();
		$stm->execute([$url]);
		$obj = $stm->fetch($fetchMode);
		if ($obj === false) return null;
	...
	public static function find_all($fetchMode=\PDO::FETCH_OBJ) {
		$sql = Sql::select("defaultAvatar")->where(['status', '=', "'1'"]);
		$stm = $sql->prepare();
		$stm->execute();
