...
class clientType implements Listable {
	...
	public static function find_all($fetchMode=\PDO::FETCH_OBJ) {
		$sql = Sql::select("clientType")->where(['status', '=', "'1'"]);
		$stm = $sql->prepare();
		$stm->execute();
    ...
    public function list($request) {
		...
		$obj = null;
		return new FormPage('clientType/list', $obj);
	}	
    ...
    public function delete($request) {	
		...
		if (!isset($request->get->id) || empty($request->get->id))
			return new Data(['success'=>false, 'message'=>L('error.clientTypeEmptyID')]);	

		$clientTypeObj = self::find($request->get->id);	
		
		if(is_null($clientTypeObj))
			return new Data(['success'=>false, 'message'=>L('error.clientTypeNotFound'), 'field'=>'notice']);		
			
		$sql = Sql::delete('clientType')->where(['id', '=', $request->get->id]);

		$logData = [];
		$logData['userID']= $currentUserObj->id;
		$logData['module'] = "Client Type";
		$logData['referenceID'] = $request->get->id;
		$logData['action'] = "Delete";
		$logData['description'] = "Delete Client Type [".$clientTypeObj->name."]";
		...
		if ($sql->prepare()->execute()) {
			return new Data(['success'=>true, 'message'=>L('info.clientTypeDeleted')]);	
		} else {
			return new Data(['success'=>false, 'message'=>L('error.clientTypeDeleteFailed')]);	
		}					
	}    

    public function clientTypeForm($request) {
		...
		$formName = "form-addClientType";

		if(!is_null($obj)) {
			$formName = "form-editClientType";
		}				

		$content = "<form id='".$formName."' class='' autocomplete='off'>";
		$content .= "<div class='row'><p class='col-md-12 col-lg-12 text-primary' id='notice'>".L('info.clientTypeAddHelperMessage')."</p></div>";

		$content .= "<div class='row'>";        
			$content .= formLayout::rowInputNew(L('clientType.name'),'name', 'name', 'text',  6, [], ['required'], is_null($obj)?'':$obj['name']);
    ...
    public function add($request) {	
		...
		if (!isset($request->post->name) || empty($request->post->name)) 
			return new Data(['success'=>false, 'message'=>L('error.clientTypeEmptyName'), 'field'=>'name']);

        // insert database
		$sql = Sql::insert('clientType')->setFieldValue([
            'name' => "?",
		...
		if ($sql->prepare()->execute($addValues)) {
			...
			$logData['description'] = "Create New Client Type [".$request->post->name."]";
			$logData['sqlStatement'] = $sql;
    ...
    public function edit($request) {	
		...
		if (!isset($request->get->id) || empty($request->get->id))
			return new Data(['success'=>false, 'message'=>L('error.clientTypeEmptyID'), 'field'=>'notice']);
		...
		if (isset($request->post->name) && !empty($request->post->name)) {
			...
			if($request->post->name!=$clientTypeObj->name) {
				$logContent[] = [
					"key"=>"name", 
					"valueFrom"=>$clientTypeObj->name, 
					"valueTo"=>strip_tags($request->post->name)					
				];	
			}			
		}	
		...
		if (isset($request->post->status) && !empty($request->post->status)) {
			...
			if($request->post->status!=$clientTypeObj->status) {

				$logContent[] = [
					"key"=>"status", 
					"valueFrom"=>$clientTypeObj->status, 
		...
		$sql = Sql::update('clientType')->setFieldValue($editFields)->where(['id', '=', $request->get->id]);
		...
		if ($sql->prepare()->execute($editValues)) {
			if (count($logContent)) {
				...
				$logData['description'] = "Edit Client Type [".$clientTypeObj->name."]";
				$logData['sqlStatement'] = $sql;
	...
	public static function genTableHeader() {
        ...
        $htmlContent .= "<thead>";
            $htmlContent .= "<tr>";
				...
				$htmlContent .= "<th>".L('clientType.name')."</th>";
                $htmlContent .= "<th>".L('clientType.totalClient')."</th>";
	...
	public static function genTableContentData() {
		$sql = Sql::select(['clientType', 'clientType'])->leftJoin(['status', 'status'], "clientType.status = status.id");
		$sql->setFieldValue('
		   clientType.id id, 
	...
	public static function genTableBodyRow($listObj) {
        ...
        $htmlContent .= "<tr>";
			...
			$htmlContent .= "<td>".$listObj['clientTypeName']."</td>";
            $htmlContent .= "<td>".self::getTotalClientByType($listObj['id'])."</td>";
	...
	public static function getTotalClientByType($clientTypeID) {

		$sql = Sql::select("client")->where(['status', '=', "'1'"])->where(['clientTypeID', '=', $clientTypeID]);
		$stm = $sql->prepare();
		$stm->execute();
    ...
    public function detail($request) {
		...
		$content = "<div class='row'>";        
			$content .= formLayout::rowDisplayLineNew(L('clientType.name'), $obj['name'], 6);
