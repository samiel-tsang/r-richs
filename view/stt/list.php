            ...
            <div class="col-md-12">
               ...
               <div class="card-header">
                  <div class="d-flex align-items-center">
                      <h4 class="card-title"><?=L('menu.stt');?></h4>
                      <button class="btn btn-primary btn-round ms-auto addSttBtn">
               ...
               <div class="card-body">
                  <div class="table-responsive">
                     <table id="sttTable" class="display table table-striped table-hover dataTable">
                     <?=Controller\stt::genTableHeader();?>
                        <?=Controller\stt::genTableFooter();?>
                        <tbody>
                           <?php
                           $content = Controller\stt::genTableContentData();
                           foreach($content as $listObj) {
                              echo Controller\stt::genTableBodyRow($listObj);                                    
                           } ?>
                        </tbody>
                     </table>
                  </div>
...
<script>
      $(document).ready(function () {
         ...
         function autoTab(tabID) {
            $(".sttAddMenuA").removeClass('active');
            $(".sttAddMenuDiv").removeClass('show active');
         ...
         var sttTable = $("#sttTable").DataTable({
               ...
               serverSide: true,
               serverMethod: 'post',
               ajax: '<?=$request->baseUrl();?>/script/sttList.php?mode=edit',
               "columns": [
                  { data: 'column_sttID' },
                  { data: 'column_refNo' },
                  { data: 'column_tpbNo' },
         ...
         $(".addSttBtn").click(function(e){
               ...
               e.preventDefault();
               ajaxFunc.apiCall("GET", "stt/formAdd", null, null,  function (form_data) { 
         ...
         $('#sttTable tbody').on('click', '.btnView', function (e) {
            e.preventDefault();
            var button = $(e.currentTarget);
            show_stt_detail(button.data('id'));
         });   


         $('#sttTable').on('click', 'tbody tr td:not(:last-child)', function(e) {
            e.preventDefault();
            show_stt_detail($(this).parent().attr('id'));                 
        });            
         
         function show_stt_detail(sttID) {
            ajaxFunc.apiCall("GET", "stt/detail/"+sttID, null, null,  function (form_data) { 
               $('#msgBox').one('show.bs.modal', function (ev) {                 
                  var modal = $(this);
                  modal.find('.modal-dialog').addClass("modal-xl"); /* extend xl modal */
                  ...
                  if(form_data.content.success) {
                     ...
                     modal.find('#msgBoxBtnPri').on('click', function (event) {  
                        ...
                        var mailingLogTable = $("#mailingLogTable").DataTable({
                           ...
                           serverMethod: 'post',
                           autoWidth: false,
                           ajax: '<?=$request->baseUrl();?>/script/sttMailingLogList.php?mode=view&sttID='+sttID,
                           "columns": [
         ...
         $('#sttTable tbody').on('click', '.btnEdit', function (e) {
               e.preventDefault();
               var button = $(e.currentTarget);
               ajaxFunc.apiCall("GET", "stt/formEdit/"+button.data('id'), null, null,  function (form_data) { 
                  $('#msgBox').one('show.bs.modal', function (ev) {                 
                     var modal = $(this);
            ...
            $(".removeDoc").off('click').on( "click" , function(e) {
               ...
               swal({
                  ...
                  if (willDelete) {
                     ...
                     ajaxFunc.apiCall("POST", "stt/removeDoc/"+btn.attr('data-id'), {"sttID":button.data('stt'), "docType":button.data('doc')}, null, function(return_data){
