...
$stm = Database\Sql::select('generalStatus')->prepare();
$stm->execute();                                          
foreach ($stm as $opt) {  
    $statusSelectionStr .= '<option value="'.$opt['id'].'">'.$opt['name'].'</option>';
}      
?>  			
   <div class="container">
      <div class=""></div>
      ...
      $(document).ready(function () {
         ...
         function eotTable(tpbID) {
            var eotTable = $("#eotTable").DataTable({
               ...
               serverSide: true,
               serverMethod: 'post',
               ajax: '<?=$request->baseUrl();?>/script/eotList.php?tpbID='+tpbID,
                  "columns": [
                  { data: 'column_eotID' },
